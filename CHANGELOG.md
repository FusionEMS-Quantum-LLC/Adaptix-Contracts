...
- **`adaptix_contracts.application_registry` — canonical AdaptixCore
  application registry (ARCH-APP-REGISTRY-001).** The product taxonomy was
  hand-typed in five independent Adaptix-Web-App files (sidebar, command
  palette, mobile menu, keyboard chords, home cards) that disagreed with each
  other and gated on module ids the backend cannot grant. This module is the
  one list: `ApplicationDefinition` (job → canonical route → workspaces →
  ANY-OF entitlement modules → owning service audiences → device clients →
  external portals → lifecycle status → visibility), plus
  `SharedCapabilityDefinition` for Search / Notifications / Communications /
  TrustSign / Cortex and the other platform functions that are consumed by
  applications and must not be presented as applications. Twenty-three
  applications across the eleven directive domains (COMMAND … ADMINISTRATION),
  including `cct` and `citizen_patient` as `DEFERRED` and `demos` as
  `EXPERIMENTAL`. Every route was verified against Web-App main `35b4a12e`;
  every service audience against Gateway main `5658fb1c`.
  Import-time validation rejects alias module spellings (the registry can never
  mint a second entitlement vocabulary), unknown service audiences, unknown
  client repositories, two owners for one route, an `ACTIVE` application with
  no route or no owner, a `DEFERRED`/`RETIRED` application with a route, and a
  tenant-visible `EXPERIMENTAL` application. `module_registry` is untouched
  and remains the entitlement authority.
- **Pricing linkage.** `applications_unlocked_by(module_id)`,
  `capabilities_unlocked_by(module_id)` and
  `sold_products_for_application(app_id, catalog)` resolve
  sold product → entitlement module → application/workspace through
  `module_registry.expand_entitlements`, so bundle SKUs (`hems_ops`) and legacy
  aliases (`billing_automation`) land where a route gate would. A priced
  product with a module id that unlocks no surface fails the suite — the
  navigation analogue of the billable-but-dark SKU check.
- **Machine-readable catalog.** `export_application_catalog()` and the
  committed `adaptix_contracts/application_catalog.json` (regenerated by
  `scripts/export_application_catalog.py`; `--check` and
  `tests/test_application_registry.py` fail on drift). Records
  `schema_version`, `contracts_version`, `pricing_catalog_version`,
  `source_repository`, presentation `position`, and each application's
  `sold_as`. Adaptix-Web-App mirrors this file into its generated TypeScript
  catalog; nothing downstream retypes the taxonomy.
...
- **`TrustSignClient.resend_request` removed** (A005 follow-up to #220).
  ...
  contract because no active consumer needs resend: an org-wide GitHub code
  search for `resend_request` across `FusionEMS-Quantum-LLC` found only the
  method itself, its unit test, Billing-Service's independent Phase-1
  in-process mirror (a separate implementation, not a caller of this
  client), and Billing-Service's own deprecated legacy `trustsign_routes.py`
  server endpoint. A caller anywhere in the fleet that actually needs
  resend can either reissue by cancelling and re-creating the signing
  request, or a future PR can implement the full standalone server
  contract (token revoke/reissue, expiry, audit, notification, abuse
  limits). Bumped `test_trustsign_client_mirror_drift`'s
  `EXPECTED_CLIENT_METHODS` and dropped the round-trip test.

...
class LegacyIdentity:
    """Identity fields covered by the legacy Gateway HMAC."""
...
def _reject_stale_legacy_timestamp(issued_at: int, clock_skew_seconds: int) -> None:
...
def verify_legacy_identity(
    *,
    ...
    issued_at = _parse_legacy_timestamp(timestamp)
    _reject_stale_legacy_timestamp(issued_at, clock_skew_seconds)
    identity = LegacyIdentity(tenant_id=tenant_id, user_id=user_id, email=email)
    expected = _legacy_hmac_hex(
        secret,
...
# Back-compat aliases the correctly named: sign() / verify() for the modern
